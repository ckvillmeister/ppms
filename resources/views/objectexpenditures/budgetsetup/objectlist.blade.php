                ...
                @foreach ($objectexpenditures as $objectexpenditure)
                    <tr data-objid="{{ $objectexpenditure->id }}">
                        ...
                        <td class="text-center">
                            @if ($objectexpenditure->aipcode)
                                <code class="text-primary">{{ $objectexpenditure->aipcode }}</code>
                            @else
                                <code class="badge badge-primary">Double click to set AIP Code</code>
                            @endif
                        </td>
    ...
    $('#tbl_list tbody td').dblclick(function() {
        ...
        if (colindex == 4){
            ...
            $('#tbl_list tbody tr').each(function(){
                ...
                if (text == 'Double click to set AIP Code'){
            ...
            if (td.text().trim() != 'Double click to set AIP Code'){
                code = td.text().trim();
            }
    ...
    $('body').on('keypress', '#aip-code', function(e){
        ...
        if(keycode == 13){
            $.ajax({
                ...
                method: 'POST',
                data: {'deptid': deptid, 'objid': objid, 'code': code},
                setCookies: token,
                dataType: "HTML",
                beforeSend: function() {
                    $('#basicloader').show();
